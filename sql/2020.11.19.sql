...
create table Contact (
 ...
 addr varchar2(36) default 'null' not null,
 email varchar2(36) default 'null' not null,
 category varchar(12) constraint cg_check check (category in('univ','com','cafe')),
 uni_major varchar(12),
 uni_year number,
 com_companyname varchar(12),
 com_dname varchar(12),
 com_job varchar(12),
 caf_name varchar(12),
 caf_nickname varchar(12)
 );
  
--2. DEPT 테이블에 데이터를 삽입하는 SQL을 작성하시오. 입력 데이터는 임의로 작성하시오.
select * from dept;
insert into dept values(65,'마케팅','서울');
--3. DEPT 테이블에 위에서 삽입한 데이터의 dname, loc 데이터를 변경하는 SQL을 작성하시오.
--   입력 데이터는 임의로 작성하시오.
update dept set dname='전산',loc='인천' where deptno=65;
...
create or replace view emp_view as select e.ename, e.job, d.deptno, d.dname, d.loc from emp e, dept d where e.deptno=d.deptno;
