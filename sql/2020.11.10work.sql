...
select ename, case when MGR is null then 0 else mgr end as "상관 사번" from emp;
...
select max(sal),min(sal),max(sal)-min(sal) as "급여 최고액-최저액" from emp;
...
select deptno,count(empno) as "사원수",round(avg(sal),2) from emp group by deptno;
-- 30. 각 부서에 대해 부서번호 이름, 지역 명, 사원 수, 부서내의 모든 사원의 평균 급여를 출력하시오.
-- 평균 급여는 정수로 반올림 하시오. DECODE 사용.​
select deptno,round(avg(sal)) as "평균급여",count(*) as "사원수",
...
select job ,deptno as dno from emp  group by job, deptno order by dno, job;
