...
select round(sysdate-to_date('2020/01/01','YYYY/MM/DD'),0) as "1월1일부터" 
from dual;
​
--21. 사원들의 상관 사번을 출력하되 상관이 없는 사원에 대해서는 NULL 값 대신 0으로 출력하시오.
--nvl(컬럼,치환값)
select ename, nvl(mgr,0) as "상관사번"  from emp;
​
...
select deptno,count(*) as "사원수", round(avg(sal),2)
from emp
group by deptno;

--30. 각 부서에 대해 부서번호 이름, 지역 명, 사원 수, 부서내의 모든 사원의 평균 급여를 출력하시오. 평균 급여는 정수로 반올림 하시오. DECODE 사용.
select deptno, decode(deptno, 10, 'ACCOUNTING',20,'RESEARCH',30,'SALES',40,'OPERATIONS') AS DEPTNAME
, decode(deptno,10,'NEWYORK',20,'DALLAS',30,'CHICAGO',40,'BOSTON') AS LOC
, count(*) as "사원수", round(avg(sal),0)
from emp
group by deptno;
​
