...
from emp e, (select min(sal) as mins ,deptno from emp group by deptno)m 
where e.deptno=m.deptno
and e.sal=m.mins
order by e.deptno;

--48. 담당업무가 ANALYST 인 사원보다 급여가 적으면서 업무가 ANALYST가 아닌 사원들을 표시(사원번호, 이름, 담당 업무, 급여)하시오.
select empno, ename, job, sal from emp where sal < (select distinct(sal) from emp where job='ANALYST')
and job != 'ANALYST';
...
select empno,ename from emp where deptno=any(select distinct(deptno) from emp where ename like '%K%');
...
where empno=any(select empno from emp where sal> (select avg(sal) from emp))
and deptno=any(select distinct(deptno) from emp where ename like '%M%');

--58.평균급여가 가장 적은 업무
select job,avg(sal) from emp group by job having avg(sal)=(select min(avg(sal)) from emp group by job);
--59.담당업무가 MANAGER 인 사원이 소속된 부서와 동일한 부서의 사원을 표시하시오.
select deptno from emp where job='MANAGER';
select ename from emp where deptno=any(select deptno from emp where job='MANAGER');