...
public class ArrayTest2 {
	...
	public static void main(String[] args) {
		...
		System.out.println(arr.length); //3 -> 1차원 배열을 저장하는 주소값 3개 
										//arr의 length값은 2차원 배열의 행의 개수다
										//2차원 배열 arr의 length 값은 1차원 배열의 개수를 가진다.
		
		System.out.println(arr[0].length);//4 arr[0]는 첫번째 1차원 배열을 가르키는 주소값을 저장하는 변수이다.
		System.out.println(arr[1].length);//4 arr[0]는 두번째 1차원 배열을 가르키는 주소값을 저장하는 변수이다.
		System.out.println(arr[2].length);//4 arr[0]는 세번째 1차원 배열을 가르키는 주소값을 저장하는 변수이다.
...
//		}
		...
		System.out.println("2차원 배열의 모든 요소 데이터 저장======");
		// 전체 요소에 값을 대입
		for(int i = 0; i <arr.length ; i++) { //0,1,2
			for(int j = 0; j<arr[i].length;j++) { //0,1,2,3
				arr[i][j] = i+j;
		...
		System.out.println("2차원 배열의 모든 요소 데이터 출력=====");
		showData(arr); //밑과 동일
		
		//for 반복문을 이용해서 2차원 배열의 모든 요소 참조
		for(int i = 0; i <arr.length ; i++) { //0,1,2
			for(int j = 0; j<arr[i].length;j++) { //0,1,2,3
				System.out.print(arr[i][j]+"\t");
			}
	...
	//for-each 구분 : 배열의 원본 데이터의 수정이 아닌 일괄 참조의 목적일때 사용할 수 있다.
